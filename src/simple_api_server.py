...
def run_command(cmd: str, timeout: int = 30) -> tuple[int, str, str]:
    ...
    try:
        ...
        logger.info(f"Running command (timeout={timeout}s): {cmd}")
        start_time = time.time()
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=timeout)
        elapsed = time.time() - start_time
        logger.info(f"Command completed in {elapsed:.2f}s: returncode={result.returncode}, stdout_len={len(result.stdout)}, stderr_len={len(result.stderr)}")
        if result.stdout:
        ...
        elapsed = time.time() - start_time
        logger.error(f"Command timed out after {elapsed:.2f}s (timeout={timeout}s): {cmd}")
        return -1, "", f"Command timed out after {timeout} seconds"
    except Exception as e:
        logger.error(f"Command error: {e} for command: {cmd}")
        return -1, "", str(e)

def get_pod_name(devpod_name: str) -> str:
    """Get pod name for devpod - simple version"""
...
def execute_simple_task(task_id: str, devpod_name: str, github_repo: str, 
                       github_token: str, task_description: str):
    """Execute task using simple approach from successful script"""
    ...
    try:
        ...
        add_log(task_id, f"Checking if repo exists: {repo_name}")
        logger.info(f"[{task_id}] Checking repo: {check_cmd}")
        ...
        if returncode == 0:
            ...
            logger.info(f"[{task_id}] Updating repo: {update_cmd}")
            returncode, stdout, stderr = run_command(update_cmd, timeout=30)
            if returncode != 0:
            ...
            # Use a simpler approach without credentials in URL for logging
            safe_clone_cmd = f"kubectl exec -n devpod {pod_name} -- bash -c 'cd ~ && git clone https://github.com/{github_repo}.git'"
            logger.info(f"[{task_id}] Cloning repo (safe log): {safe_clone_cmd}")
            
            # Actual clone with credentials
            clone_cmd = f"kubectl exec -n devpod {pod_name} -- bash -c 'cd ~ && git clone https://{github_token}@github.com/{github_repo}.git'"
            returncode, stdout, stderr = run_command(clone_cmd, timeout=120)
            
            if returncode != 0:
                add_log(task_id, f"❌ Clone failed: {stderr}")
                logger.error(f"[{task_id}] Clone failed with returncode {returncode}: {stderr}")
                raise Exception(f"Failed to clone repository: {stderr}")
            else:
                add_log(task_id, f"Clone output: {stdout[:200]}")
...
if command -v claude &> /dev/null; then
    ...
    exit 1
        ...
        if returncode != 0:
            ...
            # Add Claude output to logs (last 10 lines)
            output_lines = stdout.strip().split('\\n')[-10:]
            for line in output_lines:
                if line.strip():
                    add_log(task_id, f"Claude: {line}")
        ...
        # Step 9: Check for Streamlit app and run it
        with tasks_lock:
        ...
        if "EXISTS" in stdout:
            ...
            if "EXISTS" in stdout:
                ...
                run_command(install_cmd, timeout=180)
...
if __name__ == '__main__':
    print("Starting Simplified API Server on http://localhost:15001")
